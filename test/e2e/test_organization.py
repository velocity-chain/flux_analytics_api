"""
E2E tests for Organization endpoints (consume-style, read-only).
...
def test_get_organizations_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/organization?name=test", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_get_organization_not_found():
    """Test GET /api/organization/<id> with non-existent ID."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(
        f"{BASE_URL}/api/organization/000000000000000000000000",
        headers=headers,
    )
