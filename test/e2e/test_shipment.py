"""
E2E tests for Shipment endpoints (consume-style, read-only).

These tests verify that Shipment endpoints work correctly by making
...
def test_get_shipments_endpoint():
    """Test GET /api/shipment endpoint."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/shipment", headers=headers)
...
def test_get_shipments_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/shipment?name=test", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_get_shipment_not_found():
    """Test GET /api/shipment/<id> with non-existent ID."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(
        f"{BASE_URL}/api/shipment/000000000000000000000000",
        headers=headers,
    )
...
@pytest.mark.e2e
def test_shipment_endpoints_require_auth():
    """Test that shipment endpoints require authentication."""
    response = requests.get(f"{BASE_URL}/api/shipment")
