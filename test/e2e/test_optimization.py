"""
E2E tests for Optimization endpoints (consume-style, read-only).

These tests verify that Optimization endpoints work correctly by making
...
@pytest.mark.e2e
def test_get_optimizations_endpoint():
    """Test GET /api/optimization endpoint."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/optimization", headers=headers)
...
def test_get_optimizations_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/optimization?name=test", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_get_optimization_not_found():
    """Test GET /api/optimization/<id> with non-existent ID."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(
        f"{BASE_URL}/api/optimization/000000000000000000000000",
        headers=headers,
    )
...
@pytest.mark.e2e
def test_optimization_endpoints_require_auth():
