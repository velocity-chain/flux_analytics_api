...
These tests verify that Analytics endpoints work correctly by making
...
@pytest.mark.e2e
def test_create_analytics_endpoint():
    """Test POST /api/analytics endpoint and verify record persists in database."""
    ...
    data = {
        ...
        "description": "E2E test analytics document",
    }

    response = requests.post(f"{BASE_URL}/api/analytics", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-analytics"
    assert "created" in response_data
    assert "saved" in response_data


@pytest.mark.e2e
def test_get_analyticss_endpoint():
...
def test_get_analyticss_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/analytics?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
