"""
E2E tests for Inventory endpoints (consume-style, read-only).

These tests verify that Inventory endpoints work correctly by making
...
@pytest.mark.e2e
def test_get_inventorys_endpoint():
...
def test_get_inventorys_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/inventory?name=test", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_get_inventory_not_found():
    """Test GET /api/inventory/<id> with non-existent ID."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(
        f"{BASE_URL}/api/inventory/000000000000000000000000",
        headers=headers,
    )
