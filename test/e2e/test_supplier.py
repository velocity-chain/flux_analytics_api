"""
E2E tests for Supplier endpoints (consume-style, read-only).

These tests verify that Supplier endpoints work correctly by making
...
@pytest.mark.e2e
def test_get_suppliers_endpoint():
    """Test GET /api/supplier endpoint."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/supplier", headers=headers)
...
def test_get_suppliers_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/supplier?name=test", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_get_supplier_not_found():
    """Test GET /api/supplier/<id> with non-existent ID."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(
        f"{BASE_URL}/api/supplier/000000000000000000000000",
        headers=headers,
    )
...
@pytest.mark.e2e
def test_supplier_endpoints_require_auth():
    """Test that supplier endpoints require authentication."""
    response = requests.get(f"{BASE_URL}/api/supplier")
